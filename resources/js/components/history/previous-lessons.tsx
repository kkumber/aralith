                ...
                {lessons && lessons.length && (
                    <div className="flex flex-col items-center justify-center gap-2">
                        {lessons.map((lesson: LessonResponse) => (
                            <Card
                                className={`relative w-full gap-1 ${selected.includes(lesson.id) ? 'border-primary-green bg-primary-green/5' : ''} hover:border-primary-green z-0 pt-2 pl-5 transition-all duration-200 ease-out hover:cursor-pointer hover:bg-black/5 dark:hover:bg-white/5`}
                                key={lesson.id}
                            ...
                            >
                                ...
                                <div
                                    className={`absolute top-10 left-1 z-10 transform transition-all duration-200 ease-out ${
                                        selected.includes(lesson.id) || showCheckbox.includes(lesson.id) || selected.length > 0
                                            ? 'translate-x-0 scale-100 opacity-100'
                                            : 'pointer-events-none translate-x-0 scale-75 opacity-0'
                                    }`}
                                >
                                    <div
                                ...
                                <CardContent className="pl-3">
                                    <CardTitle>
                                        <Button variant={'link'} className="p-0 text-base font-bold" asChild>
                                            <Link
                                                href={route('lesson.show', { lesson: lesson.id })}
